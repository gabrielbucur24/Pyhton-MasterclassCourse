...
def parse_invoice_number(invoice_number: str) -> tuple[int, int]:
    ...
    """
    # year = int(invoice_number[:4])
    # inv_num = int(invoice_number[5:])
    # inv_tup = [year, inv_num]
    inv_tup = [int(invoice_number[:4]), int(invoice_number[5:])]
    return tuple(inv_tup)
...
def next_invoice_number(invoice_number: str) -> str:
    ...
    """
    number = str(int(invoice_number[5:]) + 1)
    zeros = '000'
    if len(number) < 4:
        length = 4 - len(number)
        number = zeros[:length] + number
    if invoice_number[:4] == '2023':
        return invoice_number[:5] + number
    else:
        return '2023-0001'
